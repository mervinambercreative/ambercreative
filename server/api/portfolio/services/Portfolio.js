...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Portfolio.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Portfolio.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Portfolio._attributes)
      .filter(attribute => attribute !== Portfolio.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Portfolio._attributes[attribute].type));
